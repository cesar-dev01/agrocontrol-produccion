...
function Campaigns({ campaigns, loading, showCampaign, closeCampaign, editCampaign, deleteCampaign, timezone }) {
  const status = { planned: 'Planificada', preparation: 'Preparación', growing: 'En crecimiento', harvesting: 'En cosecha', closed: 'Cerrada' }
  const displayDate = date => date ? new Intl.DateTimeFormat('es-PE', { day: 'numeric', month: 'short', year: 'numeric', timeZone: timezone }).format(new Date(`${date}T12:00:00`)) : 'Sin fecha'
...
function Movements({ movements, campaigns, loading, showMovement, editMovement, deleteMovement, exportExcel, exportPdf, money, unitMoney, timezone }) {
  ...
  const displayDate = value => value ? new Intl.DateTimeFormat('es-PE', { day: 'numeric', month: 'short', year: 'numeric', timeZone: timezone }).format(new Date(value)) : ''
...
function MovementModal({ close, campaigns, products, addMovement, updateMovement, syncInventory, movement, unitMoney, currencySymbol }) {
...
function Harvests({ harvests, campaigns, loading, showHarvest, editHarvest, deleteHarvest, money, timezone }) {
  ...
  const displayDate = value => value ? new Intl.DateTimeFormat('es-PE', { day: 'numeric', month: 'short', year: 'numeric', timeZone: timezone }).format(new Date(`${value}T12:00:00`)) : ''
...
function HarvestModal({ close, campaigns, addHarvest, updateHarvest, harvest, currencySymbol }) {
...
function LiveDashboard({ products, campaigns, movements, harvests, showProduct, showCampaign, money, timezone, userName, farmName }) {
...
function Settings({ user, onUserUpdate, preferences, onSavePreferences, theme, onThemeChange }) {
  const storageKey = `agro-settings-${user.id}`
  const [form, setForm] = useState({ fullName: user.name || '', ...preferences })
  const [saving, setSaving] = useState(false)
  const [message, setMessage] = useState('')
  useEffect(() => setForm({ fullName: user.name || '', ...preferences }), [user.id, user.name, preferences.farmName, preferences.currency, preferences.timezone, preferences.notifications])
  const change = (field, value) => setForm(current => ({ ...current, [field]: value }))
  const submit = async event => {
    event.preventDefault(); setSaving(true); setMessage('')
    const nextPreferences = { farmName: form.farmName, currency: form.currency, timezone: form.timezone, notifications: form.notifications }
    if (supabase) {
      const { error } = await supabase.auth.updateUser({ data: { full_name: form.fullName } })
      if (error) { setMessage(error.message); setSaving(false); return }
      const profile = await supabase.from('profiles').upsert({ id: user.id, full_name: form.fullName, farm_name: form.farmName, currency: form.currency, timezone: form.timezone, notifications_enabled: form.notifications, theme })
    ...
    localStorage.setItem(storageKey, JSON.stringify({ ...nextPreferences, theme }))
    onUserUpdate(nextUser); onSavePreferences(nextPreferences); setMessage('Configuración guardada y sincronizada.'); setSaving(false)
  }
  return <><div className="page-heading"><div><p className="eyebrow">PREFERENCIAS</p><h1>Configuración</h1><p className="muted">Administra tu perfil y las preferencias generales de AgroControl.</p></div></div>
    <form className={accountStyles.settingsForm} onSubmit={submit}>
      ...
      <div className={accountStyles.formFooter}>{message && <p className={message.includes('sincronizada') ? accountStyles.success : accountStyles.error}>{message}</p>}<button className="primary" type="submit" disabled={saving}>{saving ? 'Guardando...' : 'Guardar cambios'}</button></div>
    </form>
  </>
}
...
function App({ children }) {
  ...
  const money = useMemo(() => new Intl.NumberFormat('es-PE', { style: 'currency', currency: preferences.currency, maximumFractionDigits: 0 }), [preferences.currency])
  const unitMoney = useMemo(() => new Intl.NumberFormat('es-PE', { style: 'currency', currency: preferences.currency, minimumFractionDigits: 2, maximumFractionDigits: 2 }), [preferences.currency])
  const currencySymbol = preferences.currency === 'USD' ? '$' : 'S/'
  const currencyLabel = `${preferences.currency} ${currencySymbol}`
  const excelMoneyFormat = preferences.currency === 'USD' ? '$ #,##0.00' : 'S/ #,##0.00'
  const longDate = date => new Intl.DateTimeFormat('es-PE', { dateStyle: 'long', timeZone: preferences.timezone }).format(date)
  const fileDate = () => new Intl.DateTimeFormat('en-CA', { timeZone: preferences.timezone }).format(new Date())
  ...
  const activateAccount = nextUser => {
    ...
    const storedPreferences = readStoredJson(`agro-settings-${nextUser.id}`, {})
    setPreferences({ ...defaultPreferences, ...storedPreferences })
    if (storedPreferences.theme) setTheme(storedPreferences.theme)
    if (supabase) {
  ...
  useEffect(() => {
    ...
    supabase.from('profiles').select('full_name, farm_name, currency, timezone, notifications_enabled, theme').eq('id', ownerId).single().then(({ data, error }) => {
      if (error || !data || activeUserId.current !== ownerId) return
      const nextPreferences = { farmName: data.farm_name || '', currency: data.currency || 'PEN', timezone: data.timezone || 'America/Lima', notifications: data.notifications_enabled ?? true }
      const nextTheme = data.theme || 'light'
      setPreferences(nextPreferences); setTheme(nextTheme)
      localStorage.setItem(`agro-settings-${ownerId}`, JSON.stringify({ ...nextPreferences, theme: nextTheme }))
      if (data.full_name && data.full_name !== user.name) setUser(current => current?.id === ownerId ? { ...current, name: data.full_name } : current)
    })
  }, [user?.id])
  useEffect(() => {
  ...
  const exportReport = () => {
    const rows = [['Campaña', 'Parcela', 'Temporada', `Inversión (${currencyLabel})`, `Ingresos (${currencyLabel})`, `Resultado (${currencyLabel})`, 'Margen (%)'], ...reportRows().map(row => [...row.slice(0, 6).map((value, index) => index >= 3 ? Number(value).toFixed(2) : value), (row[6] * 100).toFixed(1)])]
    const content = rows.map(row => row.map(value => `"${String(value).replaceAll('"', '""')}"`).join(',')).join('\n'); const file = new Blob([`\uFEFF${content}`], { type: 'text/csv;charset=utf-8;' }); const url = URL.createObjectURL(file); const link = document.createElement('a'); link.href = url; link.download = `reporte-agrocontrol-${fileDate()}.csv`; link.click(); URL.revokeObjectURL(url)
  }
  ...
  const exportMovementsExcel = async () => {
    ...
    sheet.addRow([`Generado: ${longDate(new Date())}`]); sheet.addRow([`Inversión total: ${unitMoney.format(total)}`]); sheet.addRow([])
    const header = sheet.addRow(['Fecha', 'Campaña', 'Etapa', 'Tipo', 'Insumo', 'Descripción', 'Cantidad', 'Unidad', `Costo unitario (${currencyLabel})`, `Costo total (${currencyLabel})`]); header.font = { bold: true, color: { argb: 'FFFFFFFF' } }; header.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF226849' } }
    ...
    totalRow.font = { bold: true, color: { argb: 'FFFFFFFF' } }; totalRow.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF226849' } }; totalRow.getCell(10).numFmt = excelMoneyFormat; totalRow.alignment = { vertical: 'middle' }
    sheet.views = [{ state: 'frozen', ySplit: 5 }]
    const buffer = await workbook.xlsx.writeBuffer(); const blob = new Blob([buffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `movimientos-agrocontrol-${fileDate()}.xlsx`; link.click(); URL.revokeObjectURL(url)
  }
  const exportMovementsPdf = async () => {
    ...
    doc.setTextColor(34, 61, 48); doc.setFontSize(10); doc.text(`Generado: ${longDate(new Date())}`, 14, 39); doc.setFontSize(12); doc.text(`Inversión total: ${unitMoney.format(total)} · ${rows.length} movimientos`, 14, 48)
    ...
    doc.save(`movimientos-agrocontrol-${fileDate()}.pdf`)
  }
  const exportExcel = async () => {
    const ExcelJS = await import('exceljs'); const workbook = new ExcelJS.Workbook(); const summary = reportRows(); const now = longDate(new Date()); const reportFileDate = fileDate()
    const summarySheet = workbook.addWorksheet('Resumen'); summarySheet.columns = [{ width: 28 }, { width: 24 }, { width: 20 }, { width: 18 }, { width: 18 }, { width: 18 }, { width: 13 }]; summarySheet.mergeCells('A1:G1'); summarySheet.getCell('A1').value = 'AGROCONTROL - REPORTE DE RENTABILIDAD'; summarySheet.getCell('A1').font = { bold: true, size: 14, color: { argb: 'FFFFFFFF' } }; summarySheet.getCell('A1').fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF164A36' } }; summarySheet.addRow([`Generado: ${now}`]); summarySheet.addRow([]); const header = summarySheet.addRow(['Campaña', 'Parcela', 'Temporada', `Inversión (${currencyLabel})`, `Ingresos (${currencyLabel})`, `Resultado (${currencyLabel})`, 'Margen']); header.font = { bold: true, color: { argb: 'FFFFFFFF' } }; header.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF226849' } }; summary.forEach(row => summarySheet.addRow(row)); summarySheet.getColumn(4).numFmt = excelMoneyFormat; summarySheet.getColumn(5).numFmt = excelMoneyFormat; summarySheet.getColumn(6).numFmt = excelMoneyFormat; summarySheet.getColumn(7).numFmt = '0.0%'
    const movementSheet = workbook.addWorksheet('Movimientos'); movementSheet.columns = [{ width: 32 }, { width: 22 }, { width: 24 }, { width: 15 }, { width: 18 }, { width: 14 }, { width: 12 }, { width: 12 }, { width: 18 }, { width: 18 }]; const movementHeader = movementSheet.addRow(['Descripción', 'Campaña', 'Insumo', 'Etapa', 'Tipo', 'Fecha', 'Cantidad', 'Unidad', `Costo unitario (${currencyLabel})`, `Costo total (${currencyLabel})`]); movementHeader.font = { bold: true, color: { argb: 'FFFFFFFF' } }; movementHeader.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF226849' } }; movements.forEach(m => movementSheet.addRow([m.description, m.cycleName || '', m.productName || '', m.stage, m.activity_type, m.started_at?.slice(0, 10), Number(m.quantity || 0), m.unit || '', Number(m.unit_cost || 0), Number(m.total_cost || 0)])); movementSheet.getColumn(9).numFmt = excelMoneyFormat; movementSheet.getColumn(10).numFmt = excelMoneyFormat
    const harvestSheet = workbook.addWorksheet('Cosechas'); harvestSheet.columns = [{ width: 26 }, { width: 14 }, { width: 14 }, { width: 14 }, { width: 18 }, { width: 36 }]; const harvestHeader = harvestSheet.addRow(['Campaña', 'Fecha', 'Cantidad', 'Unidad', `Ingreso (${currencyLabel})`, 'Observaciones']); harvestHeader.font = { bold: true, color: { argb: 'FFFFFFFF' } }; harvestHeader.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF226849' } }; harvests.forEach(h => harvestSheet.addRow([h.cycleName || '', h.harvested_on, Number(h.quantity || 0), h.unit, Number(h.total_income || 0), h.notes || ''])); harvestSheet.getColumn(5).numFmt = excelMoneyFormat
    const buffer = await workbook.xlsx.writeBuffer(); const blob = new Blob([buffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `reporte-agrocontrol-${reportFileDate}.xlsx`; link.click(); URL.revokeObjectURL(url)
  }
  const exportPdf = async () => {
    const { jsPDF } = await import('jspdf'); const { default: autoTable } = await import('jspdf-autotable'); const doc = new jsPDF({ unit: 'mm', format: 'a4' }); const now = longDate(new Date()); const rows = reportRows(); const totalCost = rows.reduce((sum, row) => sum + row[3], 0); const totalIncome = rows.reduce((sum, row) => sum + row[4], 0)
